...
def main(config):
    ...
    head_model_multi = config.init_obj('head_model_multi', module_head)
    logger.info(feature_model)
    logger.info(reason_model)
    logger.info(head_model)
    logger.info(head_model_multi)
    ...
    if config.resume is None:
        if 'pretrained' in config['feature_model']:
            if config['feature_model']['pretrained']['cotrain']:
                key = config['feature_model']['pretrained']['key']
                checkpoint = torch.load(config['feature_model']['pretrained']['path'])['feature_state_dicts'][key]
            else:
        ...
        if 'pretrained' in config['head_model']:
            if config['head_model']['pretrained']['cotrain']:
                key = config['head_model']['pretrained']['key']
                checkpoint = torch.load(config['head_model']['pretrained']['path'])['head_state_dicts'][key]
            else:
        ...
        if 'pretrained' in config['head_model_multi']:
            if config['head_model_multi']['pretrained']['cotrain']:
                checkpoint = torch.load(config['head_model_multi']['pretrained']['path'])['head_state_dicts']['CVR']
            else:
                checkpoint = torch.load(config['head_model_multi']['pretrained']['path'])['head_state_dict']
            if list(checkpoint.keys())[0].startswith('module.') and config['n_gpu'] == 1:
                checkpoint = {k[7:]: v for k, v in checkpoint.items()}
            if config['n_gpu'] > 1:
                head_model_multi = torch.nn.DataParallel(head_model_multi)
                if not list(checkpoint.keys())[0].startswith('module.'):
                    checkpoint = {'module.' + k: v for k, v in checkpoint.items()}
            head_model_multi.load_state_dict(checkpoint)
    else:
        ...
        head_model_multi.load_state_dict(head_state_dict)
    ...
    head_model_multi = head_model_multi.to(device)
    feature_model.eval()
    reason_model.eval()
    head_model.eval()
    head_model_multi.eval()
    ...
    with torch.no_grad():
        for i, (data, target) in enumerate(tqdm(data_loader)):
            ...
            output_multi = head_model_multi(concept)
            output = torch.argmax(output, dim=1)
            
            # print(output, output_multi, target)
            # exit(0)
            #
            ...
            loss = loss_fn(output_multi, output)
            batch_size = data.shape[0]
            total_loss += loss.item() * batch_size
            for i, metric in enumerate(metric_fns):
                total_metrics[i] += metric(output_multi, output) * batch_size
